...
## Fallback when `computer_use` is unavailable

If the dedicated tool is not present but the user explicitly requested control of a native macOS app, use Accessibility-driven AppleScript as a constrained fallback:

1. Confirm the target process and that `System Events` UI scripting is enabled.
2. Inspect the app's live menu item names instead of guessing shortcuts.
3. Prefer clicking an exact menu item over sending blind keystrokes.
4. For long Unicode or multiline text, write it to a temporary UTF-8 file, load it with `pbcopy`, then paste.
5. Capture a post-action screenshot and inspect it with vision; an AppleScript exit code of 0 does not prove the visible action succeeded.
6. Keep the same safety boundaries as `computer_use`: never enter secrets, approve permission dialogs, or confirm destructive actions.

See `references/applescript-ui-fallback.md` for commands and a verified ChatGPT macOS example.
