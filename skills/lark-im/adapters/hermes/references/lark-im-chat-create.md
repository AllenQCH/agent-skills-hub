...
When the user asks to create a group, always use the **two-step flow** below. **Always pass `--as bot` explicitly** when calling `+chat-create` — if you omit it, the CLI may resolve to `user` identity from defaults/auto-detect and fail validation with `resolved identity "user" ... this command only supports: bot`. Do NOT pass other users' open_ids in `--users` during group creation — the bot and target users are often mutually invisible (error 232043).
