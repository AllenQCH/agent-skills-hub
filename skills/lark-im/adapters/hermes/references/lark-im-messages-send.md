...
> **Output quirk:** `+messages-send` does **not** support `--format json`. Passing `--format` fails with `unknown flag: --format`. The command already prints JSON on success, so parse stdout directly if you need `message_id` / `chat_id`. 
