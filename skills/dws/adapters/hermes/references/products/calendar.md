...
- 时间格式: `event create/update`、`event list`、`busy search`、`room search` 用 ISO-8601，建议显式带时区如 `+08:00`
- 创建日程不会自动预定会议室，需额外 `room add`
- `room search` 不带 `--group-id` 时查根目录；企业会议室超过 100 条会报错，此时需先 `room list-groups` 获取分组，再按分组逐一查询
- 查询某天某几个会议室的全天空闲情况时，不要用一个长时间窗直接判断；长窗口可能返回不可用的占位结构。应按 30 分钟粒度循环 `room search --available true`，按 `roomName` 过滤目标会议室，再合并连续 slot。详见 [calendar-room-availability.md](../calendar-room-availability.md)
- `room search` 的布尔参数建议显式传值，例如 `--available true`，不要写成裸 flag
